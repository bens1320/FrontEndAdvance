...
describe('Promise', () => {
  ...
  it('2.2.4 在我的代码执行完之前， 不得调用 then 后面的 resolve 函数', done => {
    const succeed = sinon.fake();
    const promise = new Promise(resolve => {
      resolve();
    });
  ...
  it('2.2.4 在我的代码执行完之前， 不得调用 then 后面的 reject 函数', done => {
    const fail = sinon.fake();
    const promise = new Promise((resolve, reject) => {
      reject();
    });
  ...
  it('2.2.6. then 可以在同一个promise上多次调用。resolve', done => {
    const promise = new Promise(resolve => {
      resolve();
    });
  ...
  it('2.2.6. then 可以在同一个promise上多次调用。reject', done => {
    const promise = new Promise((resolve, reject) => {
      reject();
    });
    const callbacks = [sinon.fake(), sinon.fake(), sinon.fake()];
  ...
  it('2.2.7.1.如果 then(success, fail) 中的 success 返回一个值x，运行 [[Resolve]](promise2, x)。', () => {
